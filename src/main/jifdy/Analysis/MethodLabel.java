package Analysis;

import ASTnodes.SecLabel;

import java.util.List;

/**
 * Stores security label information for a method, including parameter labels and return label.
 */
public class MethodLabel {
    public List<SecLabel> paramLabels;
    public SecLabel returnLabel;

    /**
     * Constructs a methodLabel with specified parameter and return labels.
     * @param paramLabels Labels for each parameter.
     * @param returnLabel Label for the return value.
     */
    public MethodLabel(List<SecLabel> paramLabels, SecLabel returnLabel) {
        this.paramLabels = paramLabels;
        this.returnLabel = returnLabel;
    }
}
